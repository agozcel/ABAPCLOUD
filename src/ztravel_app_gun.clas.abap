CLASS ztravel_app_gun DEFINITION
  PUBLIC
  FINAL
  CREATE PUBLIC .
...
CLASS ztravel_app_gun IMPLEMENTATION.
 METHOD if_oo_adt_classrun~main.
   DATA:itab TYPE TABLE OF ztravel_gun.
...
*   delete existing entries in the database table
   DELETE FROM ztravel_gun.

*   insert the new table entries
   INSERT ztravel_gun FROM TABLE @itab.

*   check the result
   SELECT * FROM ztravel_gun INTO TABLE @itab.
   out->write( sy-dbcnt ).
